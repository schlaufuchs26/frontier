...
A guessing game about land borders. Each round names one country; you click
every country that shares a land border with it on the world map. Eight rounds,
90 seconds each, then a debrief that names what you missed.
...
- **Rounds.** 8 targets, no repeats, never the same subregion twice in a row.
  Islands are clickable any time but never asked about (no land border to find).
...
- **Debrief.** The reveal colours the map: green found, amber missed, red wrong
  pick. Overseas and non-sovereign neighbours (French Guiana, Gibraltar, Hong
  Kong) are called out separately and never count.
- **Map.** SVG paths for every country, drag to pan, wheel or buttons to zoom.
  Each round flies to the target region; micro-states get a dot marker so they
  stay clickable.
...
src/game/state.ts           map colouring per phase
src/components/WorldMap.tsx SVG map: pan, zoom, picks, markers
